...
export type TokenType = 'partial' | 'full';
export type RegistrationStatus = 'uncomplete' | 'awaiting_approval' | 'approved' | 'complete';

// Administrator Auth Types
export interface AdminRegisterRequest {
  name: string;
...
export interface CompanyProfileRequest {
  ...
  company_logo: File;
  taxid: string;
  foundeddate: string; // DD-MM-YYYY
  companytype: string;
  companydescription: string;
}

export interface PinRequest {
  userpin: string; // 6 digit
}

...
export class PengelolaAuthService {
   ...
   */
  static async verifyOtpRegister(
    phone: string,
    otp: string
    ...
    try {
      ...
      throw {
        message:
          error.response?.data?.meta?.message || "OTP verification failed",
        status: error.response?.status || 500,
        data: error.response?.data
      };
    }
   ...
   */
  static async checkApprovalStatus(): Promise<
  ...
  static async verifyPin(userpin: string): Promise<ApiResponse<AuthResponse>> {
    try {
      ...
      throw {
        message:
          error.response?.data?.meta?.message || "PIN verification failed",
        status: error.response?.status || 500,
        data: error.response?.data
      };
    }
...
/**
 * Store authentication tokens - FIXED VERSION
 */
...
/**
 * Clear stored tokens - FIXED VERSION
 */
static clearTokens(): void {
  if (typeof window !== 'undefined') {
    ...
    // ✅ FIX: Clear cookies for middleware
